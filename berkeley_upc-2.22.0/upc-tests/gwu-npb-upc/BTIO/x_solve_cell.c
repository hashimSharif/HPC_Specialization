/*--------------------------------------------------------------------
  NAS Parallel Benchmarks 2.3 UPC version - BT source code tree
  2004 - GWU - HPCL -- check bt.c for complete information
  --------------------------------------------------------------------*/

#include <stdio.h>
#include <upc_relaxed.h>
#include "functions.h"

void x_solve_cell( int first, int last, int c )
{
  /*--------------------------------------------------------------------
    --------------------------------------------------------------------*/

  /*--------------------------------------------------------------------
    c performs guassian elimination on this cell
    c 
    c assumes that unpacking routines for non-first cells
    c preload C' and rhs' from previous cell.
    c 
    c assumed send happens outside this routine, but that
    c c'(IMAX) and rhs'(IMAX) will be sent to next cell
    --------------------------------------------------------------------*/
  int i, j, k, isize, jsize, ksize, istart;

  istart = 0;
  isize = cell_size[c][0] - 1;
  jsize = cell_size[c][1] - endc[c][1] - 1;
  ksize = cell_size[c][2] - endc[c][2] - 1;

  /*--------------------------------------------------------------------
    c outer most do loops - sweeping in i direction
    c-------------------------------------------------------------------*/
  if( first == 1 )
    {
      for( j=startc[c][1]; j<=jsize; j++ )
	{
	  for( k=startc[c][2]; k<=ksize; k++ )
	    {
	      /*--------------------------------------------------------------------
		c multiply c(istart,j,k) by b_inverse and copy back to c
		c multiply rhs(istart) by b_inverse(istart) and copy to rhs
		c-------------------------------------------------------------------*/
	      binvcrhs( &lhs_priv_d(c,istart+1,j+1,k+1,BB,0,0),
			&lhs_priv_d(c,istart+1,j+1,k+1,CC,0,0),
			&rhs_priv_d(c,istart+1,j+1,k+1,0) );
	    }
	}
    }

  /*--------------------------------------------------------------------
    c begin inner most do loop
    c do all the elements of the cell unless last
    c-------------------------------------------------------------------*/
  for( i=istart+first; i<isize-last; i++ )
    {
      for( j=startc[c][1]; j<=jsize; j++ )
	{
	  for( k=startc[c][2]; k<=ksize; k++ )
	    {
	      /*--------------------------------------------------------------------
		c rhs(i) -= A*rhs(i-1)
		c-------------------------------------------------------------------*/
	      matvec_sub( &lhs_priv_d(c,i+1,j+1,k+1,AA,0,0),
			  &rhs_priv_d(c,i-1+1,j+1,k+1,0),
			  &rhs_priv_d(c,i+1,j+1,k+1,0) );
	      /*--------------------------------------------------------------------
		c B(i) -= C(i-1)*A(i)
		c-------------------------------------------------------------------*/
	      matmul_sub( &lhs_priv_d(c,i+1,j+1,k+1,AA,0,0),
			  &lhs_priv_d(c,i-1+1,j+1,k+1,CC,0,0),
			  &lhs_priv_d(c,i+1,j+1,k+1,BB,0,0) );
	      /*--------------------------------------------------------------------
		c multiply c(i,j,k) by b_inverse and copy back to c
		c multiply rhs(i,j,k) by b_inverse(i,j,k) and copy to rhs
		c-------------------------------------------------------------------*/
	      binvcrhs( &lhs_priv_d(c,i+1,j+1,k+1,BB,0,0),
			&lhs_priv_d(c,i+1,j+1,k+1,CC,0,0),
			&rhs_priv_d(c,i+1,j+1,k+1,0) );
	    }
	}
    }

  /* for the following, i=isize-last */
  if( first == 0 )
    {
      //printf("TH%02d: wait (%d, %d)\n", MYTHREAD, first, last ); 
      upc_wait;
    }
  
  for( j=startc[c][1]; j<=jsize; j++ )
    {
      for( k=startc[c][2]; k<=ksize; k++ )
	{
	  //--------------------------------------------------------------------
	  //c rhs(i) -= A*rhs(i-1)
	  //c-------------------------------------------------------------------
	  matvec_sub( &lhs_priv_d(c,i+1,j+1,k+1,AA,0,0),
		      &rhs_priv_d(c,i-1+1,j+1,k+1,0),
		      &rhs_priv_d(c,i+1,j+1,k+1,0) );
	  //--------------------------------------------------------------------
	  //c B(i) -= C(i-1)*A(i)
	  //c-------------------------------------------------------------------
	  matmul_sub( &lhs_priv_d(c,i+1,j+1,k+1,AA,0,0),
		      &lhs_priv_d(c,i-1+1,j+1,k+1,CC,0,0),
		      &lhs_priv_d(c,i+1,j+1,k+1,BB,0,0) );
	  //--------------------------------------------------------------------
	  //c multiply c(i,j,k) by b_inverse and copy back to c
	  //c multiply rhs(i,j,k) by b_inverse(i,j,k) and copy to rhs
	  //c-------------------------------------------------------------------
	  binvcrhs( &lhs_priv_d(c,i+1,j+1,k+1,BB,0,0),
		    &lhs_priv_d(c,i+1,j+1,k+1,CC,0,0),
		    &rhs_priv_d(c,i+1,j+1,k+1,0) );
	}
    }
  i++;

  /*--------------------------------------------------------------------
    c Now finish up special cases for last cell
    c-------------------------------------------------------------------*/
  if( last == 1 )
    {
      for( j=startc[c][1]; j<=jsize; j++ )
	{
	  for( k=startc[c][2]; k<=ksize; k++ )
	    {  
	      /*--------------------------------------------------------------------
		c rhs(isize) -= A*rhs(isize-1)
		c-------------------------------------------------------------------*/
	      matvec_sub( &lhs_priv_d(c,isize+1,j+1,k+1,AA,0,0),
			  &rhs_priv_d(c,isize-1+1,j+1,k+1,0),
			  &rhs_priv_d(c,isize+1,j+1,k+1,0) );
	      /*--------------------------------------------------------------------
		c B(isize) -= C(isize-1)*A(isize)
		c-------------------------------------------------------------------*/
	      matmul_sub( &lhs_priv_d(c,isize+1,j+1,k+1,AA,0,0),
			  &lhs_priv_d(c,isize-1+1,j+1,k+1,CC,0,0),
			  &lhs_priv_d(c,isize+1,j+1,k+1,BB,0,0) );
	      /*--------------------------------------------------------------------
		c multiply rhs() by b_inverse() and copy to rhs
		c-------------------------------------------------------------------*/
	      binvrhs( &lhs_priv_d(c,i+1,j+1,k+1,BB,0,0),
		       &rhs_priv_d(c,i+1,j+1,k+1,0) );
	    }
	}
    }
}

