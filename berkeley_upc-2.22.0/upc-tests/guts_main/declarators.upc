...
{
    /*
     * To check that all static non-array shared-qualified objects have affinity with thread zero
     */

    int affinity;

    affinity = upc_threadof(&a);
    if(affinity != 0)
        GULA_FAIL("failed to setup the correct affinity for static shared variable");
