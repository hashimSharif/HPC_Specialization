...
{
    /*
     * To check that program can be terminated by a call to the function upc_global_exit
     *
     * It returns a non-zero exit code, so it is a negative test.
     */

    upc_global_exit(3);       /* Terminate the program */

    GULA_PRINT_FAIL("Failed to terminate the program with a non-zero exit code.");
