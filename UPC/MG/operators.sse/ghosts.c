//==================================================================================================
inline extract_from_grid(int low_i, int low_j, int low_k, int dim_i, int dim_j, int dim_k, int pencil, int plane, double * __restrict__ fromGrid, double * __restrict__ toBuf, int useCacheBypass){
  ...
  int i,j,k,b=0,ijk;
  fromGrid += (low_i + low_j*pencil + low_k*plane);
  if( (!useCacheBypass) || (((uint64_t)fromGrid)&0xF) || (((uint64_t)toBuf)&0xF) || (dim_i&0x1) ){
    for(k=0;k<dim_k;k++){
    for(j=0;j<dim_j;j++){
    for(i=0;i<dim_i;i++){
    ...
    for(i=0;i<dim_i;i+=2){
      int ijk = i + j*pencil + k*plane;
      _mm_stream_pd(toBuf+b,_mm_load_pd(fromGrid+ijk));
      b+=2;
    }}}
    _mm_mfence();
  }
...
inline insert_into_grid(int low_i, int low_j, int low_k, int dim_i, int dim_j, int dim_k, int pencil, int plane, double * __restrict__ fromBuf, double * __restrict__ toGrid, int useCacheBypass){
  ...
  int i,j,k,b=0,ijk;
  toGrid += (low_i + low_j*pencil + low_k*plane);
  if( (!useCacheBypass) || (((uint64_t)toGrid)&0xF) || (((uint64_t)fromBuf)&0xF) || (dim_i&0x1) ){
    for(k=0;k<dim_k;k++){
    for(j=0;j<dim_j;j++){
    for(i=0;i<dim_i;i++){
      int ijk = i + j*pencil + k*plane;
    ...
    for(i=0;i<dim_i;i+=2){
      int ijk = i + j*pencil + k*plane;
      _mm_stream_pd(toGrid+ijk,_mm_load_pd(fromBuf+b));
      b+=2;
    }}}
    _mm_mfence();
  }
...
void __box_grid_to_surface_bufs(box_type *box, int grid_id){ // i.e. extract the (non ghost zone) surface of a box
  //printf("__box_grid_to_surface_bufs\n");  
...
void __box_ghost_bufs_to_grid(box_type *box, int grid_id){ // i.e. set the ghost zone values of a box
  //printf("__box_ghost_bufs_to_grid\n");
  int ghosts = box->ghosts;
  int pencil = box->pencil;
  int plane  = box->plane;
  int dim_i = box->dim.i;
