...
void residual_and_restriction(domain_type *domain, int level_f, int phi_id, int rhs_id, int level_c, int res_id, double a, double b, double h){
  ...
  for(box=0;box<domain->numsubdomains;box++){
    ...
    double * __restrict__ temp   = domain->subdomains[box].levels[level_f].grids[__temp  ] + ghosts_f*plane_f + ghosts_f*pencil_f + ghosts_f;
    if(omp_within_a_box)  temp   = domain->subdomains[  0].levels[level_f].grids[__temp  ] + ghosts_f*plane_f + ghosts_f*pencil_f + ghosts_f; // risky, tries to get reuse across boxes by reusing box 0's __temp

    #pragma omp parallel private(k,j,i) if(omp_within_a_box)
    {
      // calculate the unrestricted residual (__temp), loop over fine dimensions, and collapse(2)
      #pragma omp for collapse(2)
      for(k=0;k<dim_k_f;k++){
      for(j=0;j<dim_j_f;j++){
      for(i=0;i<dim_i_f;i++){
        int ijk = i + j*pencil_f + k*plane_f;
        double helmholtz =  a*alpha[ijk]*phi[ijk]
                           -b*h2inv*(
                              beta_i[ijk+1       ]*( phi[ijk+1       ]-phi[ijk         ] )
                             -beta_i[ijk         ]*( phi[ijk         ]-phi[ijk-1       ] )
                             +beta_j[ijk+pencil_f]*( phi[ijk+pencil_f]-phi[ijk         ] )
                             -beta_j[ijk         ]*( phi[ijk         ]-phi[ijk-pencil_f] )
                             +beta_k[ijk+plane_f ]*( phi[ijk+plane_f ]-phi[ijk         ] )
                             -beta_k[ijk         ]*( phi[ijk         ]-phi[ijk-plane_f ] )
                            );
        temp[ijk] = rhs[ijk]-helmholtz;
      }}}
      // restrict __temp, loop over coarse dimensions, and collapse(2)
      #pragma omp for collapse(2)
      for(k=0;k<dim_k_c;k++){
      for(j=0;j<dim_j_c;j++){
      for(i=0;i<dim_i_c;i++){
        ...
        res[ijk_c] = ( temp[ijk_f                   ]+temp[ijk_f+1                 ] +
                       temp[ijk_f  +pencil_f        ]+temp[ijk_f+1+pencil_f        ] +
                       temp[ijk_f           +plane_f]+temp[ijk_f+1         +plane_f] +
                       temp[ijk_f  +pencil_f+plane_f]+temp[ijk_f+1+pencil_f+plane_f] ) * 0.125;
      }}}
    }
  }
}
