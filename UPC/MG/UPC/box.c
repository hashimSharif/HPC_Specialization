...
//------------------------------------------------------------------------------------------------------------------------------
#include "defines.h"
extern print_all_box_ptrs(box_type*);
//------------------------------------------------------------------------------------------------------------------------------
...
//printf("%2d^2 = %5d -> %5d\n",box->dim_with_ghosts.i,(dim_j+2*ghosts)*box->pencil,box->plane);
  ...
  //if(dim_i>=32){while( ((box->volume %  512) != 176) )box->volume+=8;} // 32KB / 8way / 8bytes
...
#ifdef NO_PACK
  posix_memalign((void**)&(box->surface_bufs[0]),64,total_bufsize*sizeof(double));
  memset(box->surface_bufs[0],'X',total_bufsize*sizeof(double));
  memory_allocated += total_bufsize*sizeof(double);
  
  box->ghost_bufs_sh[0] = upcr_shared_to_pshared(upcr_alloc(total_bufsize*sizeof(double)));
  box->ghost_bufs[0] = upcr_pshared_to_local(box->ghost_bufs_sh[0]);
  memset(  box->ghost_bufs[0],'y',total_bufsize*sizeof(double));
  memory_allocated += total_bufsize*sizeof(double);
  set_ghost_bufs(box);
#else 
  posix_memalign((void**)&(box->surface_bufs[0]),64,total_bufsize*sizeof(double));
  memset(box->surface_bufs[0],'a',total_bufsize*sizeof(double));
  memory_allocated += total_bufsize*sizeof(double);
  posix_memalign((void**)&(  box->ghost_bufs[0]),64,total_bufsize*sizeof(double));
  memset(  box->ghost_bufs[0],'b',total_bufsize*sizeof(double));
  memory_allocated += total_bufsize*sizeof(double);
  double *base;
  ...
  posix_memalign((void**)&(box->RedBlackMask),64,box->plane*sizeof(uint64_t));memset(box->RedBlackMask,0,box->plane*sizeof(uint64_t));
  ...
  for(i=0-ghosts;i<box->dim.i+ghosts;i++){
    ...
    if((i^j)&0x1)box->RedBlackMask[ij]=~0;else
                 box->RedBlackMask[ij]= 0;
  }}
  //for(j=0-ghosts;j<box->dim.j+ghosts;j++){
  //for(i=0-ghosts;i<box->dim.i+ghosts;i++){int ij = (i+ghosts) + (j+ghosts)*box->pencil;printf("%d",box->RedBlackMask[ij]);}printf("\n");}printf("\n");

  //print_all_box_ptrs(box);
