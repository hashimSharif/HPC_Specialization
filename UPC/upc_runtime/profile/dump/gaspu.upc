/*   $Source: bitbucket.org:berkeleylab/upc-runtime.git/profile/dump/gaspu.upc $ */
...
void gaspu_init(int *pmythread, int *pthreads) {
  *pmythread = MYTHREAD;
  *pthreads = THREADS;
  if (gaspu_zero) gasp_init(GASP_MODEL_UPC,NULL,NULL); /* this is never called - just used to force libgasp linkage */
}
void gaspu_barrier() {
  upc_barrier;
}
  ...
  void gaspu_ticks_now(gasp_tick_t *pval)  {
    *pval = (gasp_tick_t)upc_ticks_now();
  }
  void gaspu_ticks_to_sec(gasp_tick_t ticks, double *pval) {
    *pval = upc_ticks_to_ns((upc_tick_t)ticks)/1E9;
  }
...
#else /* assume UPC 1.3 and Berkeley timers unavailable */
  #include <time.h>
  #include <sys/time.h>
  void gaspu_ticks_now(gasp_tick_t *pval)  { 
    struct timeval tv;
    retry:
    if (gettimeofday(&tv, NULL)) { perror("gettimeofday"); return; }
