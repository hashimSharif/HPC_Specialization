...
shared[N] char a[N*THREADS];

int
main()
{
    /*
     * To check that the upc_memset function copies a given value 
     * converted to an unsigned char, to a shared memory area
     */

    int i;

    upc_memset(&a[N*MYTHREAD], '*', sizeof(char)*N);

    for(i=0; i<N; i++) {
        if(a[i + N*MYTHREAD] != '*')
            GULA_FAIL("failed to upc_memset properly");
    }
