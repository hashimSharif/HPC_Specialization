...
void full_verify(){
    ...
    for( i=0; i<total_local_keys; i++ ){
        key_buff1_shd[MYTHREAD+((key_buff2[i]-min_key_val)*THREADS)] --;
        key_array[key_buff1_shd[MYTHREAD+((key_buff2[i]-min_key_val)*THREADS)] - total_lesser_keys] = key_buff2[i];
    }
    ...
    if( j != 0 )
        ...
        passed_verification_shd[MYTHREAD]++;
}
...
void rank( int iteration ){
    ...
    /*  Initialize */
    upc_memset(&bucket_size_shd[MYTHREAD], 0, (NUM_BUCKETS + TEST_ARRAY_SIZE)
            * sizeof(INT_TYPE)) ;
    upc_memset(&bucket_size_totals_shd[MYTHREAD], 0, (NUM_BUCKETS
                + TEST_ARRAY_SIZE) * sizeof(INT_TYPE)) ;
    ...
    for( i=0; i<TEST_ARRAY_SIZE; i++ )
        if( (test_index_array[i]/NUM_KEYS) == MYTHREAD )
            bucket_size_shd[MYTHREAD + ((NUM_BUCKETS+i)*THREADS)] = key_array[test_index_array[i] % NUM_KEYS];

    /*  Determine the number of keys in each bucket */
    for( i=0; i<NUM_KEYS; i++ )
        bucket_size_shd[MYTHREAD + ((key_array[i] >> shift)*THREADS)]++;

    /*  Accumulative bucket sizes are the bucket pointers */
    bucket_ptrs[0] = 0;
    for( i=1; i< NUM_BUCKETS; i++ )
        bucket_ptrs[i] = bucket_ptrs[i-1] + bucket_size_shd[MYTHREAD + (i-1)*THREADS];

    /*  Sort into appropriate bucket */
    for( i=0; i<NUM_KEYS; i++ ){
        key = key_array[i];
        key_buff1_shd[MYTHREAD+(bucket_ptrs[key >> shift]++)*THREADS] = key;
    }

    upc_memcpy( &bucket_size_totals_shd[MYTHREAD],
            &bucket_size_shd[MYTHREAD],
    ...
    upc_reduced_sum( &bucket_size_totals_shd[MYTHREAD],
    ...
    send_infos_shd[0][MYTHREAD].displ = 0 ;
    process_bucket_distrib_ptr1[0] = 0;

    for( i = 0, j = 0; i < NUM_BUCKETS; i++ ){
        bucket_sum_accumulator += bucket_size_totals_shd[MYTHREAD + (i * THREADS)];
        local_bucket_sum_accumulator += bucket_size_shd[MYTHREAD + (i * THREADS)];
        if( bucket_sum_accumulator >= (j+1)*NUM_KEYS ){
            send_infos_shd[j][MYTHREAD].count = local_bucket_sum_accumulator;

            if( j != 0 ){
                send_infos_shd[j][MYTHREAD].displ =
                    send_infos_shd[j-1][MYTHREAD].displ + send_infos_shd[j-1][MYTHREAD].count ;
    ...
    for(i = 0; i < THREADS; i++){
        upc_memget(key_buff2 + total_displ,
                key_buff1_shd + i + send_infos_shd[MYTHREAD][i].displ * THREADS,
                send_infos_shd[MYTHREAD][i].count * sizeof(INT_TYPE));

        total_displ += send_infos_shd[MYTHREAD][i].count;
    }
    ...
    upc_memset( &key_buff1_shd[MYTHREAD], 0,
            (max_key_val - min_key_val + 1) * sizeof(INT_TYPE)) ;
    ...
    for( k=0; k<MYTHREAD; k++ )
        for(i=process_bucket_distrib_ptr1[k];
                ...
                i++)
            m += bucket_size_totals_shd[MYTHREAD + (i * THREADS)];
            /*  m has total # of lesser keys */
    ...
    for( i=process_bucket_distrib_ptr1[MYTHREAD];
            ...
            i++)
        j += bucket_size_totals_shd[MYTHREAD + (i * THREADS)];
        /* j has total # of local keys   */
    ...
    key_buff1_shd[MYTHREAD] += m;
    for( i=min_key_val; i<max_key_val; i++ )
        key_buff1_shd[MYTHREAD + ((i+1-min_key_val)*THREADS)] +=
            key_buff1_shd[MYTHREAD + ((i-min_key_val)*THREADS)];

    /* This is the partial verify test section */
    /* Observe that test_rank_array vals are   */
    /* shifted differently for different cases */
    for( i=0; i<TEST_ARRAY_SIZE; i++ ){
        k = bucket_size_totals_shd[MYTHREAD + (i+NUM_BUCKETS)*THREADS];
        if( min_key_val <= k  &&  k <= max_key_val ){
            kk = key_buff1_shd[MYTHREAD + ((k-1-min_key_val)*THREADS)];
#if (CLASS == 'S')
            if( i <= 2 ){
                if( kk != test_rank_array[i]+iteration )
...
#if (CLASS == 'W')
            if( i < 2 ){
                if( kk != test_rank_array[i]+(iteration-2) )
                    printf( "Failed partial verification: "
                            "iteration %d, processor %d, test key %d\n",
                            iteration, MYTHREAD, i );
                else
...
#if (CLASS == 'A')
            if( i <= 2 ){
                if( kk != test_rank_array[i]+(iteration-1) )
                    printf( "Failed partial verification: "
                            "iteration %d, processor %d, test key %d\n",
                            iteration, MYTHREAD, i );
                else
...
int main( int argc, char **argv ){
    ...
    passed_verification_shd[MYTHREAD] = 0;
    ...
    if( MYTHREAD == 0 ){
        ...
        passed_verification = passed_verification_shd[0];
