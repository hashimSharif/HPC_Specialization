...
int __umalloc_aligned_insert(struct mdesc *mdp, void *exact, void *aligned) {
    ...
    if (mdp->aligned_block_table == NULL) { /* first call - create table */
      ...
      blockno_t newtablesz = mdp->aligned_block_tablesz*2 + 1;
      blockno_t i;
      ...
      for (i=0; i < mdp->aligned_block_tablesz; i++) {
        ...
        while (cur) {
          ...
          blockno_t key = ((uintptr_t)(cur->aligned))%newtablesz;
    ...
    /* insert */
    { blockno_t key = ((uintptr_t)aligned)%mdp->aligned_block_tablesz;
