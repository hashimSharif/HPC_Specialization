                           ...
                           USE_ITT_BUILD_ARG(void *itt_sync_obj) )
{
    ...
    for (level=0, offset=1; offset<num_threads; level+=branch_bits, offset<<=branch_bits)
    {
	  ...
	  /*hash: expecting the worker thread to be entering 
	     if(!KMP_MASTER_TID(tid))
              printf("worker thread signalling its parent \n");
            else
              printf("*** master thread signalling its parent \n");
	  */
             ...
             child++, child_tid+=(1 << level))
        {
            ...
            /*hash: expecting the master thread to go into blocking waiting for the child worker threads
	     if(KMP_MASTER_TID(tid))
              printf("master thread waiting for child worker threads \n");
            else
              printf("*** worker thread waiting for God knows what \n");*/
            ...
            if (reduce) {
                ...
                //hash: expecting the master thread to go into blocking waiting for the child worker threads
	        /*if(KMP_MASTER_TID(tid))
                  printf("master thread reduces \n");
                else
                  printf("\n*** worker thread number %d reduces ! \n", tid);
		*/   
                            ...
                            USE_ITT_BUILD_ARG(void *itt_sync_obj) )
{
    ...
    if (KMP_MASTER_TID(tid)) { // master
        ...
        flag.wait(this_thr, TRUE
                  USE_ITT_BUILD_ARG(itt_sync_obj) );
        printf("*** A child thread is released with gtid  %d\n", gtid);
...
# endif // KMP_REVERSE_HYPER_BAR
                ...
                // Release child from barrier
                kmp_flag_64 flag(&child_bar->b_go, child_thr);
                printf("** Releasing child from tid %d\n ", gtid);
                flag.release();
            }
        }
    }
