...
#if OMPT_SUPPORT
    int tid = __kmp_tid_from_gtid( gtid ); //hash: is this required in the split call? How is tid different from gtid
...
__kmpc_fork_call2(ident_t *loc, kmp_int32 argc, kmpc_micro microtask, ...)
{
  ...
  // maybe to save thr_state is enough here
  {
    ...
    SSC_MARK_FORKING();
#endif
    __kmp_fork_call2( loc, gtid, fork_context_intel,
            argc,
#if OMPT_SUPPORT
            VOLATILE_CAST(void *) microtask,      // "unwrapped" task
#endif
...
/* TODO: revert workaround for Intel(R) 64 tracker #96 */
  ...
  /* --- moving this part of the code in kmp_fork_call_end 

    //--- start of imported code from kmp_internal_fork and kmp_fork_call --- 
    printf("--- * --- _kmp_fork_barrier call from kmpc_fork_call --- * --- \n");                 
  ...
  /*  if (! team->t.t_invoke( gtid )) {
      ...
      printf(" $$$$ microtask called by parent $$$$$ \n");	   
    }

    KMP_MB();       /* Flush all pending memory write invalidates.  */   
    // -- end of code from kmp_fork_call and kmp_internal_fork -----  
...
void __kmpc_fork_call_end(){

    ident_t * loc = NULL;
    int gtid = __kmp_entry_gtid();
    printf("-GTID_2=%d", gtid);
    //--- start of imported code from kmp_internal_fork and kmp_fork_call --- 
    printf("--- * --- _kmp_fork_barrier call from *kmpc_fork_call_end* --- * --- \n");                 
    __kmp_fork_barrier( gtid, 0 ); 
    kmp_info_t *master_th = __kmp_threads[ gtid ];  
    kmp_team_t *team = master_th->th.th_team;
...
__kmpc_reduce_nowait(
  ...
  /* if(reduce_func)
      printf("%d - thread is passed reduction function \n", global_tid);
    else
      printf("%d -*** thread doesn't have reduce function send as argument \n", global_tid);
  */
    ...
    if( packed_reduction_method == critical_reduce_block ) {
      ...
      //hash: printf("*** kmp_barrier condition called \n");
